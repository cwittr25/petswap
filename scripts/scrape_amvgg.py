...
`alt` attribute -- the common pattern for value-list sites, since alt
text usually just says the item's name for accessibility/SEO. This is a
best-effort heuristic: I couldn't inspect AMVGG's actual page structure
from this sandbox (no network access to the site), so if alt text turns
out not to carry the name reliably, the match-rate warning printed at the
end will make that obvious on the first real run, and the extraction
logic can be tightened from there (e.g. positional matching instead of
alt-text) once we see real output.
"""
import re
import json
import sys
...
ITEM_RE = re.compile(
    r"(?P<name>.+?)Value(?P<value>[\d.]+)Demand(?P<stars>[★☆]+)FRNM"
